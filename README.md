...
`run_race_native_buy_realtime.sh` 는 가장 공격적인 실전 수신 경로입니다. `race` 백엔드로 Telethon, TDLib, Pyrogram 중 먼저 온 이벤트를 Python/C++ fast buyer로 처리하면서, TDLib 쪽은 동시에 C++ relay 내부 native-buy도 켭니다. 즉 Telethon/Pyrogram이 먼저 받으면 기존 race 경로가 먼저 주문하고, TDLib가 먼저 받으면 Python 왕복 없이 C++ relay가 직접 주문합니다. 세 경로(Python poller, race C++ ultra engine, TDLib C++ relay) 모두 같은 공지에 대해 동일한 orderLinkId prefix(`ls-upbit-...` / `ls-bithumb-...`)를 쓰므로, 두 경로가 같은 공지를 동시에 잡아도 Bybit 중복 orderLinkId 방어가 경로 간에 정상 작동해 중복 주문이 방지됩니다. 가장 보수적인 단일 주문 경로가 필요하면 `run_tdlib_native_buy_realtime.sh` 를 씁니다.
...
- 중복 방지를 위해 세 경로 모두 동일한 `orderLinkId=ls-upbit-message_id-ticker` 또는 `ls-bithumb-message_id-ticker` 형식을 사용함:
  - Python poller (`src/poller.py`의 `_exchange_code`)는 거래소 이름을 그대로 prefix로 씀
  - race C++ ultra engine (`cpp/listing_ultra_engine.cpp`의 `order_link_exchange_code`)도 동일
  - TDLib C++ relay native-buy 경로(`cpp/tdlib_json_relay.cpp`의 `order_link_exchange_code` 및 `classify_listing_title_into`의 `order_link_exchange`)도 전체 거래소 이름(`upbit`/`bithumb`)을 prefix로 써 동일하게 맞춤
- 세 경로의 orderLinkId prefix가 일치하므로, 같은 공지를 TDLib native 와 Python/C++ ultra 가 동시에 잡아도 Bybit 중복 orderLinkId 방어가 경로 간에 정상 작동해 중복 주문이 방지됨 (이전에 TDLib relay만 `ls-u-`/`ls-b-` 단문자 prefix를 써 경로 간 dedupe가 깨지던 문제는 수정됨). 단일 주문만 보장하려면 여전히 `run_tdlib_native_buy_realtime.sh`(tdlib 단독 백엔드)를 사용할 수 있음
